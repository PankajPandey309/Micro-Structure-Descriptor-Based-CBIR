...
img_dir = r"C:\Users\HP\Desktop\CBIR\Project\CBIR-Using-MSD-master\CBIR-Using-MSD-master\test"
Database = np.zeros(45*72).reshape(45,72)
...
for f1 in files:    
    ...
    sheet1.write(entry, 0, f1)        
    for i in range(CSA):
        hist[i] = (MS[i] * 1.0) / (8.0 * HA[i] + 0.0001)
        sheet1.write(entry, i+1, hist[i])
        
    wb.save('data_final.xls') 
    Database[entry] = hist
    print("Entered for !"+f1)
    print(Database[entry])
        

file=open("data2.txt","w+")
#file.write("distances:")
myString = ','.join(map(str, Database)) 
file.write(myString)
file.close()
print (Database[0])
